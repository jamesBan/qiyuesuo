...
class Helper
{
     ...
     *
     * @access public
     * @param string $url 必填 API地址
     * @param array $heads 发送头信息 为空
     * @param array $data post方式下需要,需要urldecode
     * @version     1.0
     * @return mixed
     */
    public static function getHttps($url, $heads, $data = null)
    {
        try {
            if (function_exists('curl_init')) {
                ...
                if ($data) {
                    curl_setopt($curl, CURLOPT_POST,
                        1);//启用时会发送一个常规的POST请求，类型为：application/x-www-form-urlencoded，就像表单提交的一样。
    ...
    public static function getMillistime()
    {
        $microtime = microtime();
     ...
     * @return array
     */
    public static function checkRemoteSignUrl($postData, $type)
    {
        if (!array_key_exists("documentId", $postData)) {
            return array(
                'code' => 1005,
                'message' => '未指定合同文件的唯一标识'
            );
        }
        if (!array_key_exists("operation", $postData)) {
            return array(
                'code' => 1005,
                'message' => '未指定操作类型'
            );
        }
        if (!array_key_exists("signer", $postData)) {
            return array(
                'code' => 1005,
                'message' => '未指定签署人'
            );
        } else {
            $array_signer = json_decode($postData['signer'], true);
            if ($postData['operation'] === 'SIGNWITHPIN') {
            ...
            if (!array_key_exists("successUrl", $postData)) {
                return array(
                    'code' => 1005,
    ...
    public static function checkRemote($post_data, $type)
    {
        ...
        if ($type === 'signbyplatform' || $type === 'signbycompany' || $type === 'signbyperson') {
            ...
            if ($type === 'signbycompany') {
                if (!array_key_exists("company", $post_data)) {
                    ...
                    if (!array_key_exists("registerNo", $array_company) && !array_key_exists("email",
                            $array_company) && !array_key_exists("telephone", $array_company)) {
                        return array(
                            'code' => 1005,
            ...
            if ($type === 'signbyperson') {
                if (!array_key_exists("person", $post_data)) {
                    ...
                    if (!array_key_exists("idcard", $array_person) && !array_key_exists("email",
                            $array_person) && !array_key_exists("mobile", $array_person)) {
                        return array(
                            'code' => 1005,
                            'message' => 'registerNo，email，telephone三选一,不能都为空'
                        );
    ...
    public static function check($post_data, $type)
    {
        ...
        foreach ($receiver_array as $r) {
            ...
            if ((array_key_exists("receiveType",
                        $post_data) && $post_data['receiveType'] === 'SEQ') || !array_key_exists("receiveType",
                    $post_data)) {
                if (!is_int($r['ordinal'])) {
                    return array(
                        'code' => 1005,
    ...
    public static function checkDown($documentId)
    {
    ...
    public static function checkSign($post_data, $type)
    {
        if ($type === 'signStand') {
            if (!array_key_exists("documentId", $post_data)) {
                return array(
...
//根据返回的二进制数据（如图像）和字符数据  写到$path路径
    public static function checkDownloadAndReturn($output, $path)
    {
        //判断是否返回文件流
